...
## ⚠️ 專案脈絡已變更（2026-09-03）

本文件原本是四人團隊在公司黑客松的規劃書。**現在這是個人的 side project，
在外網開發，與公司無關。** 因此以下段落只有歷史參考價值，不是現在的約束：

| 段落 | 現況 |
|---|---|
| §1 網路環境限制、內外網分流 | 不適用。全部在外網開發 |
| §1.2 LLM 通道決策、「金融環境合規」 | 不適用。LLM 走 Azure OpenAI 公開端點 |
| §2 人員分工（D1／D2／P／T） | 不適用。單人開發 |
| §11 七天計畫、停損點 | 不適用。無時程壓力 |

**仍然有效的**是技術規格本身：§3 介面契約、§4 API 端點、§5 演算法、
§6 資料模型、§8 配額控管、§9 降級路徑、§10 前端合規要求。

實作與本文件的落差，以及開發過程中踩到的坑，記錄在專案根目錄的 `NOTES.md`。
...
兩邊的配額計數器互不知情，加起來才是 Google 端的實際用量，
熔斷門檻會因此失準（見 `NOTES.md` #24）。
