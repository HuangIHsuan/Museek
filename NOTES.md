...
| 推薦回傳 | 以為含特徵與 popularity | 特徵沒有，要再查一次；~~popularity 根本不存在~~ **見下方修訂** |

> **2026-09-07 修訂：popularity 其實有回。** 查曲風時重測 payload，
> 搜尋與推薦端點都帶了 `popularity`（實際值 84／62／58）。下面 34a 的結論
> 因此作廢——但程式碼一直寫死 `None`，所以 novelty **實際上真的失效了**，
> 只是原因是我們自己寫死的，不是對方沒給。已修，見 #49。

### ⚠️ 34a　novelty 失去資料來源（原因記錯了，見上方修訂）
...
## 47　ReccoBeats 的速率限制實測，以及「並行化」為什麼行不通

**官方文件**（`/docs/documentation/rate-limiting`）說：限制數字「configured internally」
不公開，超量回 429，並警告 *may lead to temporary blocking*，要求照 `Retry-After` 退避。
回應標頭沒有任何 `X-RateLimit-*`，所以只能實測。

### 實測數字（同一份 50 首歌單，清空快取）

| 設定 | 結果 |
|---|---|
| 並行 1（16 個請求短爆發） | 12.3s，全部 200 |
| 並行 4 | 5/16 收到 429，`Retry-After: 15` |
| 並行 8 | **16/16 全部 429**，`Retry-After: 12` |
| 間隔 0.125s（8 req/s，錯開送出） | 16 個全部 200 |

短爆發下錯開送出可以到 8 req/s，**但持續 50 個請求會撞到另一層限制**：

| 節流設定 | 50 首歌單耗時 | 429 次數 |
|---|---|---|
| 0.15s／6 個在飛 | **98 秒**（前 25 首只花 21 秒，之後被退避拖垮） | 12 |
| 0.25s／1 個在飛 | 65 秒 | 4 |
| 0.30s／2 個在飛（採用） | 55 秒 | 10 |

**結論：加大並行不會變快，只會換成 429 加退避。** 我試過並且回退了。

### 真正有效的三件事

1. **批次 audio-features**：`/v1/audio-features?ids=a,b,c` 一次 40 個。
   原本每首歌兩次請求（search + features），50 首 = 100 次；
   改成 50 次 search + 2 次批次 = 52 次。
2. **快取查不到的結果**（TTL 24 小時）。這是最大的一筆——
   查不到的曲目原本不寫快取，所以每次重新解析同一份歌單，
   那些失敗查詢（含 iTunes 補救）全部重跑。
   實測重新解析：**46.9 秒 → 0.5 秒**。
   TTL 設 24 小時而不是永久，是因為特徵之後可能補得上，長期記下來會擋住重試。
3. **節流器**（`app/services/http.py` 的 `Pacer`）：管制送出間隔與同時在飛數，
   並在 429 時照 `Retry-After` 把後續請求一起往後推。

### 仍未解決

第一次解析 50 首仍要 55 秒，瓶頸是 ReccoBeats 的限制本身，不是我們的程式。
search 端點沒有批次版本，50 首就是 50 次請求。
**因此進度條不是暫時的權宜之計，是必要的**——這個等待時間短期內消不掉。

> 註：今天為了量測打了不少請求，可能已經被歸到較嚴格的配額桶，
> 上面的秒數可能偏悲觀。乾淨環境下應該會好一些。

## 48　批次預熱 feature_cache：用歌手撈，不要一首一首搜

解析歌單慢的根因是**每首歌都要對外查一次**（NOTES #47）。與其加快查詢，
不如讓歌單進來時就已經在快取裡。
...
| 做法 | 請求數／首 |
|---|---|
| 每首歌 `track/search` 一次（解析歌單時目前的做法） | 1.04 |
| `artist/{id}/track` 一次撈整個曲庫 + 批次 audio-features | **0.062** |

`artist/{id}/track` 一次回最多 100 首，`audio-features` 又能 40 個 id 一批。

### 實際成果

用專案種子池裡的 98 位歌手跑 `scripts/prewarm_features.py`：

```
6,908 首特徵、427 次請求（每首 0.062 次）
最終 feature_cache：5,716 筆真實特徵、涵蓋 100 位歌手
```

解析 Bon Iver 官方頻道歌單：**55 秒 → 0.7 秒，ReccoBeats 請求 0 次。**

> 誠實註記：那次測試同時受惠於 #47 的 miss 快取（該歌單之前解析過）。
> 單獨衡量預熱的覆蓋率：種子池 377 首裡有 **328 首（87%）**進了快取。

### 一個必須處理的坑

`artist/search` 是**模糊比對，會撈到完全不相干的人**——實測搜 "Beach House"
回的是 "Sunset Chill" 的曲目。不驗證歌手名的話會白花請求、也沒提高命中率。
腳本裡的 `_same_artist` 就是擋這個。

### 還沒做的

- **種子池的 377 首本來就帶特徵，卻沒有寫進 feature_cache**——
  它們只當推薦種子用。啟動時載進快取是零成本的，值得補。
- 預熱寫到哪個儲存後端要看環境變數。目前跑在記憶體版會落到
  `data/cache.json`，**線上的 Firestore 讀不到**。要讓部署受惠，
  必須設 `STORAGE_BACKEND=firestore` 再跑一次。

---

## 49　「數值差不多但曲風不一樣」——曲風是缺的那一軸，不是第七個維度

### 症狀

使用者反映：推薦回來的歌，六個音訊維度跟品味向量都很接近，但**曲風完全是
另一回事**——想聽 citypop，回來的是數值一樣的 soft rock。

### 這不是排序算錯，是少了一軸

§5.1 的相似度只認得 energy／valence／danceability／acousticness／
instrumentalness／tempo。citypop 跟 soft rock、R&B 跟 lo-fi 在這六個數字上
本來就重疊——距離函式算得完全正確，它只是沒有「曲風」這個東西可以量。

**曲風不能塞成第七個維度。** 那六個是連續且可比的，歐氏距離對它們有意義；
曲風是類別，硬編碼進同一個距離要憑空發明一個尺度（citypop 到 R&B 的「距離」
是 0.3 還是 0.7？沒有答案）。所以它獨立成一項分數、獨立配名額——
跟 #46 的地區比重同一個做法。

### 真正的難題是資料：候選池裡根本沒有曲風

實測（2026-09-07）ReccoBeats 的四個端點：

| 端點 | genre 欄位 |
|---|---|
| `/v1/track/search` | 無 |
| `/v1/artist/search`、`/v1/artist/{id}` | 無（只有 id／name／spotify href） |
| `/v1/track/recommendation` | 無 |

（#46 也量過推薦端點的 `genre` 查詢參數「完全沒作用」。）所以曲風只能從外面補。

### ⚠️ 49a　選過兩次來源：Spotify 是死的，最後用 iTunes

**第一次選了 Spotify，錯了。** 理由當時看起來很強：ReccoBeats 每一首候選的
payload 都附了 Spotify 歌手 id（`artists[].href`），一次 `/v1/artists?ids=`
可以吃 50 位，而且只有 Spotify 有 `city pop` 這一級的字串。

拿到憑證實測之後才發現**那條路已經封了**：

```
POST /api/token                    -> 200（token 拿得到）
GET  /v1/artists?ids=<one id>      -> 403 Forbidden
GET  /v1/artists/<id>              -> 200，但欄位只剩
                                      external_urls / href / id / images / name / type / uri
```

**`genres` 這個鍵根本不在回傳裡**（`popularity`、`followers` 也一起消失），
批次端點直接 403。這不是憑證設錯——token 拿得到、單一端點也通，就是欄位被拔掉了。

教訓跟 #4 是同一個：**「文件上有這個欄位」不等於「這把憑證拿得到這個欄位」。**
應該在寫任何整合之前先用真憑證打一次，而不是等整條線接完才發現。
...
|---|---|---|---|
| Spotify | —— | —— | **不可用**，見上 |
| MusicBrainz | 細（SZA → neo-soul／alternative rnb） | 差，而且**會配錯人** | 否 |
| **iTunes** | 中（`R&B/Soul`、`華語流行樂`、`成人當代`） | 好 | **可** |

MusicBrainz 被排除的關鍵不是粒度，是**它會很有信心地配錯人**：
`Fujii Kaze` 配到 `Kaze` 並回 southern hip hop、`Sunset Rollercoaster` 配到
`Rollercoaster`，兩個都是 score=100。再加上限速 1 req/s、實測一半請求回 503。

**最後用 iTunes**（已經接好、免金鑰）。實測 46 位歌手收到 40 種曲風字串，
關鍵發現是 **TW 商店回中文、US 商店回英文，是同一套概念的兩種寫法**：

```
華語流行樂 = Mandopop     嘻哈/饒舌 = Hip-Hop/Rap    另類音樂 = Alternative
R&B/靈魂樂 = R&B/Soul     日本流行樂 = J-Pop         成人當代 = Adult Contemporary
```

別名表就是照這 40 個實際字串建的，不是照文件猜的（40 個裡認得 37 個，
剩下 `節慶`／`Bollywood`／`Worldwide` 本來就是雜訊，正確地被丟掉）。

實測 20 位歌手：**19 位標得到，冷啟 1.6 秒，熱機 0.000 秒**（曲風查的是歌手、
不是曲目，所以同一位歌手的多首歌只花一趟）。

### iTunes 的代價，講清楚

**它表達不出 citypop。** 山下達郎回 `J-Pop`、落日飛車回 `成人當代`。
所以加了一個 `adult_contemporary` slug，並把 `city_pop ↔ adult_contemporary`
的相似度設成 0.60（比其他鄰居高）——「想聽 citypop」目前就是靠這條邊命中的。
R&B、嘻哈、獨立搖滾、J-Pop、K-Pop、華語流行都分得出來，最細的那個圈子分不出來。

**歌手名對不上就是漏掉，不硬湊。** iTunes 對「Jay Chou」回的是「周杰倫」，
`same_artist` 拒絕把羅馬拼音跟中文名畫上等號，所以周杰倫查不到曲風。
這是刻意的：放寬驗證就會變成 MusicBrainz 那種錯配，而錯配的曲風會被當成事實
拿去排序、還會佔掉曲風名額。漏掉一位的代價是 0（查不到不受罰），配錯的代價是排錯。

### 四條必須守住的性質（少一條就會安靜地失效）

1. **使用者要的那一側不補上位曲風。** 「想聽 citypop」若被展開成
   `{city_pop, pop}`，而 `genre_fit` 取 max，那**任何一首泛泛的流行歌都是滿分**
   ——等於這個功能沒開。（`genres.normalize(parents=False)`）
2. **候選那一側要補整條上位鏈。** 一首 shoegaze 就是 indie rock、也就是 rock，
   三層都是事實；只標最細的那層，「想聽搖滾」對不上它。
3. **查不到曲風的候選不受罰。** `genre_fit` 回 `None` 時，那一項**連同權重
   一起從加權平均裡拿掉**，不是當 0 分。當 0 分的話，iTunes 查不到的候選
   （沒收、名字對不上、或這一輪額度用完）一律先扣掉 `weight_genre` 那一截，
   排序會退化成「比誰查得到曲風」——而查不查得到跟好不好聽完全無關。
4. **兩個名額不互相拆台。** 曲風名額先跑、地區名額後跑，後者要拿到
   `prefer_keep`（＝曲風剛保下來的別動）。沒有它，實測後跑的那輪會把先跑的
   擠出去，最後**兩個下限都不成立**（tests/test_genres.py 有這條的迴歸測試）。

### 「不要嘻哈」走分級，不是全丟

跟「不要太吵」同一種話，所以走 §5.4 同一層分級：違反的排到後段當備位，
候選池太小時仍然湊得滿五首。而且**只認完全命中**——「不要嘻哈」把 lo-fi
一起排掉是過度解讀，硬過濾一旦過度解讀，候選池會安靜地少一大塊。

### 順帶修掉的：novelty 一直是個常數

查曲風時重測 payload，發現**推薦端點其實有回 `popularity`**（實際值 84／62／58），
但 `_parse_track` 照 #34 寫死 `None`。後果是 §5.3 的
`novelty = 1 - popularity/100` 對每一首都用 0.5——**`weight_novelty` 那 25%
的分數是常數，對排序零貢獻**。#46 尾段記過這件事但沒改，現在照實收了。

### 兩道閘：stub 模式與每輪額度

曲風是掛在推薦流程上的**額外對外請求**，所以有兩個開關，少一個都出過事：

1. **`RECCOBEATS_MODE=stub` 一律不查。** stub 的意思就是完全不對外。
   少了這道閘，整份測試會安靜地打到 iTunes——而且因為 `_search` 自己把例外
   吞掉，看起來只會像「就是比較慢」。實測：測試從 10 秒變成 **150 秒**，
   而唯一的線索是 log 裡的 `iTunes 搜尋失敗：測試不該對外連線`。
2. **`GENRE_LOOKUP_MAX`（預設 25）**：一輪最多對外查幾位**新**歌手。
   沒有上限的話，一輪 50 首全是新歌手就是 50 個往返疊在使用者的等待上。
   超過額度的那幾位這一輪就是沒有曲風——不受罰，下一輪快取就有了。

另外查詢是 `asyncio.gather` 同時發出的，節奏交給節流器（iTunes 用 0.05 秒
間隔、4 個同時在飛）。**這裡不能沿用 ReccoBeats 的 0.3 秒**——實測那樣
25 位歌手替每一輪加了十幾秒。

### 還沒做的

- `data/vibe_seeds.json` 要**重跑 `scripts/verify_vibe_seeds.py`** 才會帶
  `genres`。在那之前，補進來的亞洲候選在 `_draw_asia` 那一步沒有曲風可比
  （之後仍會在推薦流程裡被現場標，只是多花幾趟請求）。
- 曲風快取是**行程內**的，重啟就沒了。落到 repo（跟 feature_cache 一樣）
  的話冷啟那 1.6 秒也能省掉，但要多一個 collection。
- `RELATED` 那張跨家族相似度表是手寫的，只寫了真的沾得上邊的。寫寬一點
  「想聽 R&B」就會開始回搖滾——那是這個功能最糟的失敗方式，所以寧可漏。
- 曲風掛在**歌手**身上而不是曲目上，同一位歌手的每首歌拿到同一組標籤。
  對「這首是不是 citypop」不夠精確，但那是目前負擔得起的唯一粒度
  （也正是它便宜的原因：一位歌手一趟請求，不是一首歌一趟）。


---

## 50　「曲風可以盡量抓一樣的嗎」——權重調不動它，要動的是候選池

接 #49。曲風那一軸接上去之後，實際用起來還是常常「不是我聽的那一圈」。
查下去有兩個原因，一個是資料、一個是我自己寫錯的。

### 原因一：品味輪廓被上位曲風灌水（真的 bug）

候選身上的曲風會補整條上位鏈（`shoegaze` → `indie_rock` → `rock`），
那是為了讓「想聽搖滾」對得上一首只標 shoegaze 的歌。但統計**使用者品味**時
照單全收就出事了：

```
一份清一色獨立搖滾的歌單 → {indie_rock: 1.0, rock: 1.0}
一首 metal 的標籤        → {metal, rock}
genre_fit                → 1.0   ← 滿分命中
```

`rock` 是我們**推論**出來的，不是觀察到的，卻拿到跟 `indie_rock` 一樣的票；
而 metal 的上位也是 rock。改成統計時只算最具體的那一層（`genres.leaves`）之後：
metal 掉到 0.4（低於「算命中」的 0.5 門檻），shoegaze 仍然是 1.0——它**就是**
獨立搖滾，不是隔壁的曲風。

**兩側的需求是相反的**：描述一個人要盡量具體，比對一首歌要盡量寬。

### 原因二：候選池裡本來就沒有那個曲風（這才是主因）

實測一份真實候選池 + 一位聽 R&B 的使用者：

| 設定 | 前五同曲風 |
|---|---|
| `weight_genre=0.30`、名額 2 | 2/5 |
| `weight_genre=0.70`、名額 3 | 2/5 |
| `weight_genre=1.00`、名額 4 | **2/5** |

**權重與名額調到底，數字完全不動。** 因為 42 首候選裡只有 2 首對得上，
而且查不到曲風的候選不受罰（#49 的性質 3，那是對的）——沒東西可選，
排序當然排不出來。這跟 #46 是同一句話：**排序排不出池子裡沒有的東西。**

而執行期補不出覆蓋率：iTunes 的量級是每分鐘約 20 次（#49）。

### 改法：從自己的種子池補同曲風候選

`seed_pool.GENRES` 手工標了全部 98 位歌手（跟 `region` 一樣是我們自己的主張）。
**手工標而不是打 iTunes 的關鍵理由是第三個**：iTunes 沒有 citypop 這個分類，
山下達郎在它眼裡是 J-Pop、落日飛車是「成人當代」——這份清單是候選池裡
**唯一標得出 city_pop 的地方**，而 citypop 正是這整件事最初的那個例子。

每輪補 `GENRE_CANDIDATES`（預設 12）首曲風一定對得上的候選進池子。
補進來的歌**不享有任何加分**（同 #46 的紀律），只是讓池子裡有東西可選。

實測（6 種品味 × 8 次）：前五同曲風從 2/5 → **4/5**，五首全是不同歌手。

### 三個一定要一起做的細節，少一個就會歪掉

1. **兩區各補一半。** 種子池亞洲那區比較大（59 對 39），整池一起抽的話補進來的
   幾乎都是亞洲的，接著曲風名額把它們拉進前五 → **5/5 亞洲**，正是 #46 的
   地區上限當初要擋的「幾乎全部」。
2. **跨次抽取要去重歌手。** 注入分三次抽（亞洲、曲風西洋、曲風不限），
   每次內部有去重、跨次沒有——實測「聽 lo-fi 的人」前五拿到**三首 STUTS**。
   同溫層懲罰擋不到它：那一項只看使用者聽過的歌手，管不到同一輪之內。
3. **`_as_candidate` 的 region 缺值不能預設成 asia。** 原本只有亞洲注入時
   看不出問題；曲風注入從兩區一起抽之後，那個預設值等於把歐美的歌算成亞洲。

### ⚠️ 50a　兩個名額會打架，順序就是優先順序

「至少 4 首同曲風」與「至多 2 首亞洲」常常無法同時成立（補進來的同曲風候選
多半是亞洲的）。實測：曲風名額先跑拿到 4 首，地區上限接著把其中 2 首換掉，
**最後只剩 2 首同曲風**。

現在曲風排在後面，也就是曲風贏；地區上限退成「盡量」（帶 `prefer_keep=is_asia`，
真的要換人時先換非亞洲的）。理由是使用者感覺得到的差異：端出四首他從沒聽過的
曲風，比端出四首亞洲的歌更像是推薦壞了。

**順帶抓到兩個回報錯誤**：
- 兩個名額的數字原本各自取自己的回傳值，但**後面那一輪還會再動前段**，
  先算的那個在端出去之前就過期了。現在一律從最終的前段重算。
- `region_quota` 回傳新清單、不會就地修改，而我第一版把地區那一輪的回傳值
  丟掉了——那樣地區名額等於整個沒跑。

### 已知的缺口

- **種子池的曲風覆蓋不均。** 它當初是照**特徵空間**挑的（安靜↔吵、慢↔快），
  不是照曲風。實測 lo-fi 只有 STUTS 與 Nujabes 兩位，去重後最多補 2 首，
  所以「聽 lo-fi 的人」只拿得到 2/5。要補就是往 `POOL` 加人再重跑
  `verify_vibe_seeds.py`。
- **`data/vibe_seeds.json` 有 44 首的歌手是錯的**（`Sunset Chill`、
  `Kwanzaa Posse feat. Massive Attack`…）。那是 #48 記過的模糊比對陷阱：
